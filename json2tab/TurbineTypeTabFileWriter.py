...
class TurbineTypeTabFileWriter:
    ...
    def write_specs_file(self, matched_line_index: int, filename: str) -> dict:
        ...
        with open(filename, "w") as file:
            ...
            if source_string:
                source_string = file_with_link_info(source_string)
            if (
                "source" in specs
                and specs["source"]
            ...
            file.write(
                "# Wind turbine specification file generated by "
                f"json2tab v{version('json2tab')} "
                f"on {self.matcher.match_generated}\n"
            )
