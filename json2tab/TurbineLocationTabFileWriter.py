...
class TurbineLocationTabFileWriter:
    ...
    def __init__(self, config: Dict[str, Any], matcher: TurbineMatcher):
        ...
        try:
            self.raise_line_is_removed_warning = config["warnings"]["line_is_removed"]
        except (KeyError, ValueError, TypeError):
            self.raise_line_is_removed_warning = True
    ...
    def _write_header(self, file, subsetting_handler=None):
        ...
        file.write(
            ...
            f"json2tab v{version('json2tab')} on {self.matcher.match_generated}\n"
        )

        location_files = [
            file_with_link_info(p) for p in self.location_manager.location_files
        ]
        file.write(
            "#     Source(s) for turbine locations: - " f"{spaces.join(location_files)}\n"
        )

        if self.type_manager is not None:
            specs_files = [file_with_link_info(p) for p in self.type_manager.specs_files]
        else:
            specs_files = ["unknown (reused from existing matching)"]

        file.write(
            "#     Source(s) for type database:     - " f"{spaces.join(specs_files)}\n"
        )

        selection_method = (
            subsetting_handler.display_name()
            if subsetting_handler is not None
            else "unknown"
        )
        file.write(f"#     Subdomain selection method:      {selection_method} \n")
    ...
    def write(
        ...
        subsetting_handler: Optional[Any] = None,
        ...
        with open(output_path, "w") as file:
            # Write header
            self._write_header(file, subsetting_handler)
                ...
                try:
                    ...
                    if model_designation is not None:
                        ...
                        if nwp_passed:
                            ...
                            skipCounter += 1
                    elif self.raise_line_is_removed_warning:
                        logger.warning(
